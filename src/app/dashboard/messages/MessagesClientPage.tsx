...
export default function MessagesPage({ isAdmin, initialInternalUsers, initialMassMessages, initialLocations, initialDemographics, initialIndividualMessages, currentUserId }: MessagesPageProps) {
  const [activeTab, setActiveTab] = useState('individual-messages');
  const [messages, setMessages] = useState<Message[]>(initialIndividualMessages); // Initialize with individual messages
            ...
            {messages.length === 0 ? (
              ...
              messages.map((msg) => (
                <div key={msg.id} className="mb-4 p-3 bg-gray-100 rounded-lg cursor-pointer">
                  <p className="text-sm font-semibold">From: {msg.sender.name} To: {msg.recipient.name}</p>
