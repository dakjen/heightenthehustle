...
import { updateBusinessProfile, archiveBusiness } from "../../actions";
...
export default function EditBusinessProfileClientPage({ initialBusiness }: EditBusinessProfileClientPageProps) {
  ...
  useEffect(() => {
    if (editState?.message && !editState.error) {
      // Re-fetch business data to show updated values (if needed, or rely on revalidation)
      // For now, we'll just update the local state if the message indicates success
      // In a real app, you might re-fetch or update the state more robustly
      // For simplicity, we'll assume the form submission revalidates the path
      // and the server component will provide the latest data on next render.
      // If you need immediate client-side update, you'd pass a re-fetch function from server component.
    }
  }, [editState]);

  const handleArchive = async () => {
    if (window.confirm("Are you sure you want to archive this business? This action cannot be undone.")) {
      const result = await archiveBusiness(business.id);
